...
const registry: TemplateEntry[] = [
  ...
  },
  {
    ...
    extraFormFields: [
      {
        key: "deliveryName",
        label: "Tên nơi giao",
        placeholder: "Công Ty TNHH Kyong Gi Vina",
      },
      {
        key: "deliveryAddress",
        label: "Địa chỉ giao",
    ...
    get component() {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      return require("./templates/jesang-delivery-template")
        .JesangDeliveryTemplate;
    },
  },
  {
    id: "warehouse-export",
    ...
    get component() {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      return require("./templates/warehouse-export-template")
        .WarehouseExportTemplate;
    },
  },
  {
    ...
    defaultItems: [
      {
        productName: "Tiền cước Internet",
        customFields: { label: "Tiền cước Internet", amount: "0" },
      },
      {
        productName: "Tiền cước PayTV",
        customFields: { label: "Tiền cước PayTV", amount: "0" },
      },
      {
        productName: "Tiền thuê thiết bị đầu cuối",
        customFields: {
          label: "Tiền thuê thiết bị đầu cuối",
          amount: "0",
          hideInDntt: "1",
        },
      },
    ...
    extraFormFields: [
      ...
      { key: "position", label: "Chức vụ Bên A", placeholder: "Giám đốc" },
      {
        ...
        placeholder: "1D Nguyễn Duy, Phường Gia Định...",
      },
      {
        key: "invoiceAddress",
      ...
      },
      {
        key: "servicePackage",
        label: "Gói dịch vụ",
        placeholder: "Meta (1000Mbps/1000Mbps)",
      },
      {
        key: "servicePriceCode",
        label: "Gói tính cước",
        placeholder: "Meta-13.0T",
      },
      {
        key: "servicePrice",
        label: "Giá gói dịch vụ",
        placeholder: "4.080.000",
      },
      { key: "paymentMonths", label: "Số tháng", placeholder: "13.0" },
      {
        key: "paymentPeriodFrom",
        label: "Kỳ hạn TT từ",
        placeholder: "10/03/2026",
      },
      {
        key: "paymentPeriodTo",
        label: "Kỳ hạn TT đến",
        placeholder: "09/04/2027",
      },
    ],
    get component() {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      return require("./templates/contract-appendix-template")
  ...
  },
  {
    ...
    description: "Layout thư đề nghị thanh toán",
    shortLabel: "DNTT",
    columns: [
      { key: "label", label: "Khoản thu", type: "text", width: "55%", system: false },
      { key: "amount", label: "Số tiền (VNĐ)", type: "currency", width: "35%", align: "right" },
      { key: "hideInDntt", label: "Ẩn", type: "checkbox", width: "10%", align: "center" },
    ],
    showTotal: true,
    showProductSelector: false,
    hideCompanyDetails: true,
    ...
    extraFormFields: [
      ...
      },
      {
        ...
        placeholder: "Bốn triệu không trăm tám mươi nghìn đồng",
      },
      {
        key: "paymentDeadline",
      ...
      },
      {
        key: "paymentContent",
        label: "Nội dung CK",
        placeholder: "SGAAV3359 _ thanh toán cước",
      },
    ],
    get component() {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      return require("./templates/payment-request-template")
...
export function getExtraFormFields(
  templateId?: string | null,
): ExtraFormField[] {
...
export function getTemplateColumns(templateId?: string | null): ColumnDef[] {
  return (
    registry.find((t) => t.id === templateId)?.columns ?? registry[0].columns
  );
}
...
export function getAllCustomColumnKeys(): { key: string; label: string }[] {
  ...
  for (const t of registry) {
    for (const col of t.columns) {
      if (col.system || BUILTIN_KEYS.has(col.key) || seen.has(col.key))
        continue;
      seen.add(col.key);
