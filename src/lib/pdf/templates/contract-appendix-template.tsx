    ...
    <View style={s.partyRow}>
      ...
      <View style={s.twoColValue}>
        <View style={s.twoColLeft}>
          {typeof leftVal === "string" ? <Text>{leftVal}</Text> : leftVal}
        </View>
    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.bodyText}>
          ...
          <Text style={s.bodyTextBold}>{field(tf, "contractDate")}</Text> và các
          Phụ lục kèm theo giữa:
        </Text>
        ...
        <View style={{ marginTop: 2, marginBottom: 2 }}>
          ...
          <View style={s.partyRow}>
            ...
            <Text style={s.partyValue}>
              {field(tf, "installAddress", data.customerAddress)}
            </Text>
          </View>
          <View style={s.partyRow}>
            ...
            <Text style={s.partyColon}>:</Text>
            <Text style={s.partyValue}>
              {field(tf, "invoiceAddress", data.customerAddress)}
            </Text>
          </View>
          <TwoColPartyRow
            label="Điện thoại"
            leftVal={field(tf, "phone")}
            rightLabel="Fax"
        ...
        <Text style={s.bodyText}>
          Sau đây gọi tắt là{" "}
          <Text style={s.bodyTextBold}>&quot;Hợp đồng&quot;</Text>
        </Text>
        <Text
          style={[
            s.bodyText,
        ...
        >
          Bằng phụ lục này (&quot;Phụ lục&quot;), hai Bên cùng nhau thống nhất
          sửa đổi các điều khoản của Hợp đồng như sau:
        </Text>
        ...
        <Text style={s.bodyText}>
          Kể từ ngày{" "}
          <Text style={s.bodyTextBold}>{field(tf, "effectiveDate")}</Text> Bên A
          yêu cầu
        </Text>
        <Text style={[s.bodyText, { fontWeight: "bold", marginTop: 1 }]}>
          {" "}
        ...
        <View style={s.bullet}>
          ...
          <Text style={s.bulletText}>
            Sử dụng gói Dịch vụ: {field(tf, "servicePackage")}; Gói tính cước:
            {field(tf, "servicePriceCode")}
          </Text>
        </View>
        <View style={s.bullet}>
          <Text style={s.bulletDot}>•</Text>
          <Text style={s.bulletText}>
            Giá gói Dịch vụ:{" "}
            <Text style={s.bodyTextBold}>{field(tf, "servicePrice")} VNĐ</Text>/{" "}
            {field(tf, "paymentMonths")} Tháng.
          </Text>
        </View>
        <View style={s.bullet}>
          <Text style={s.bulletDot}>•</Text>
          <Text style={s.bulletText}>
            Kỳ hạn thanh toán từ ngày{" "}
            <Text style={s.bodyTextBold}>{field(tf, "paymentPeriodFrom")}</Text>{" "}
            đến ngày{" "}
            <Text style={s.bodyTextBold}>{field(tf, "paymentPeriodTo")}</Text>.
          </Text>
        </View>
        <View style={s.bullet}>
          <Text style={s.bulletDot}>•</Text>
          <Text style={s.bulletText}>
            Trường hợp Bên A có nhu cầu thay đổi gói Dịch vụ mà chỉ làm thay đổi
            duy nhất kỳ hạn thanh toán thì Bên B sẽ chủ động cập nhật gói Dịch
            vụ đáp ứng nhu cầu của Bên A và được xác nhận bằng Biên nhận thu
            tiền/hóa đơn mà Bên B cung cấp cho Bên A. Trường hợp hết kỳ hạn
            thanh toán của gói Dịch vụ Bên A đang sử dụng mà Bên A không tiếp
            tục thanh toán theo gói Dịch vụ đó thì được xem là Bên A tự động
            chuyển đổi sang gói Dịch vụ mới có cùng tốc độ đường truyền và các
            dịch vụ kèm theo với kỳ hạn thanh toán trả theo từng tháng.
          </Text>
        </View>
        ...
        <Text style={s.bodyText}>
          <Text style={s.bodyTextBold}>2.1.</Text>
          {"     "}Tổng số tiền Bên A thanh toán cho Bên B để thực hiện nội dung
          sửa đổi tại ĐIỀU 1 là{" "}
          <Text style={s.bodyTextBold}>
            {field(tf, "servicePrice", fmtVND(totalAmount))} VNĐ
          </Text>
        </Text>
        ...
        <View style={s.table}>
          ...
          {items.map((item, idx) => {
            ...
            const cfAmt = item.customFields?.amount
              ? Number(item.customFields.amount)
              : 0;
            const amt = cfAmt || item.amount || 0;
            return (
              <View key={idx} style={isLast ? s.tableRowLast : s.tableRow}>
                <Text style={s.tableCellLabel}>
                  {item.productName || item.customFields?.label || ""}
                </Text>
          ...
          <View style={s.tableRowLast}>
            <Text style={[s.tableCellLabelHeader, { textAlign: "left" }]}>
              Tổng tiền
            </Text>
            <Text style={s.tableCellAmount}>{fmtVND(totalAmount)}</Text>
          </View>
        </View>
        <Text style={s.tableNote}>Khoản thu đã bao gồm thuế VAT</Text>

        <Text style={s.bodyText}>
          <Text style={s.bodyTextBold}>2.2.</Text>
          {"     "}Theo quy định pháp luật, hóa đơn của Bên B phát hành hợp lệ
          không nhất thiết phải có con dấu của Bên B và Bên B lựa chọn sử dụng
          chữ viết là tiếng Việt không dấu trên hóa đơn
        </Text>
        ...
        <View style={s.bullet}>
          <Text style={s.bulletDot}>•</Text>
          <Text style={s.bulletText}>
            Bên A có trách nhiệm thanh toán khoản thu cho Bên B trong vòng 07
            (bảy) ngày kể từ ngày phát hành hóa đơn giá trị gia tăng. Trường hợp
            Bên A thanh toán quá hạn so với quy định này hoặc không đồng ý thanh
            toán, Bên A phải bồi thường 8% giá trị thanh toán của hóa đơn.
          </Text>
        </View>
        <View style={s.bullet}>
          <Text style={s.bulletDot}>•</Text>
          <Text style={s.bulletText}>
            Trong quá trình sử dụng Dịch vụ nếu Bên A muốn thay đổi gói Dịch vụ
            sử dụng hoặc bất kỳ thay đổi nào khác liên quan đến Hợp đồng đã ký
            kết, hai Bên thống nhất Phương thức giao dịch điện tử (Ví dụ: SMS,
            email, qua website fpt.vn/member, ứng dụng Hi FPT) là một trong
            những Phương thức giao dịch giữa hai bên. Phụ lục này là một phần
            không tách rời của Hợp đồng, các nội dung khác của Hợp đồng không
            được đề cập đến trong Phụ lục này là không thay đổi và giữ nguyên
            giá trị pháp lý.
          </Text>
        </View>
        <View style={s.bullet}>
          <Text style={s.bulletDot}>•</Text>
          <Text style={s.bulletText}>
            Phụ lục này được lập thành 02 bản có giá trị như nhau, mỗi Bên giữ
            01 (một) bản và có hiệu lực kể từ ngày ký hoặc ngày xác nhận Phụ lục
            thông qua Phương thức giao dịch điện tử.
          </Text>
        </View>
        ...
        <View style={s.sigRow}>
          <View style={s.sigBlock}>
            <Text style={s.sigTitle}>
              {signatureLabels[0] ?? "Đại diện bên A"}
            </Text>
            <Text style={s.sigName}> </Text>
          </View>
          <View style={s.sigBlock}>
            <Text style={s.sigTitle}>
              {signatureLabels[1] ?? "Đại diện bên B"}
            </Text>
            {company.position && (
              <Text style={s.sigPosition}>{company.position}</Text>
            )}
